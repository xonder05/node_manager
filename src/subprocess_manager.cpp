...
void SubprocessManager::StopProcess(std::string id)
{
    if (processes.find(id) != processes.end())
    {
        process &p = processes[id];

        // guard, makes sure to not kill entire os with pid -1
        ...
        auto thread = std::move(p.stdout_thread);
        processes.erase(id);
...
void SubprocessManager::RestartProcess(std::string id, std::string command, std::vector<std::string> command_params, F func, std::tuple<FParams...> func_params)
{
    StopProcess(id);
    StartProcess(id, command, command_params, func, func_params);
}